...
void testACR1281UReaderMifareClassicUnsecured(ACR1281UReader& acr1281u, BYTE startPage) {
    ...
    try {
        ...
        MifareCardCore<ACR1281UReader> card(acr1281u, false);  // 1K kart
		...
		// 7) Read sector trailers for all sectors and print their access bits (for debugging)
		// card.loadAllSectorConfigsFromCard(); // This will read the trailers and populate the card's sectorConfigs based on the access bits it finds
        card.printAllTrailers();

        // 8) ensure default sector config: KeyA read-only, KeyB read+write
        MifareCardCore<ACR1281UReader>::SectorKeyConfig defaultCfg(true, false, true, true);
		std::vector<MifareCardCore<ACR1281UReader>::SectorKeyConfig> sectorConfigs(16, defaultCfg);
		sectorConfigs.at(0).keyB_canWrite = false; // Sector 0: KeyA read-only, KeyB read-only
		card.applyAllSectorsConfig(sectorConfigs);

        // 9) Try a pre-read of the start page using card (safe)
        {
        ...
        // 12) Optionally, print full dump using reader's own readAll (if you prefer original behaviour)
