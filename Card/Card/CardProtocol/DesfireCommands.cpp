...
Result<StatusWord, PcscError> DesfireCommands::evaluateResponse(const BYTEV& response) {
	...
	// sw.isSuccess() ? R::Ok(sw) : R::Err(PcscError::from(Iso7816Error::Unknown, "Desfire error: " + sw.getMessage()));
	if (sw1 == SW1 && (sw2 == OK || sw2 == MORE)) return R::Ok(sw);
	...
	switch (sw2) {
		...
		case NO_APP:
		    return R::Err(Error<PcscError>(DesfireError::AppNotFound));
	    case NO_FILE:
		    return R::Err(Error<PcscError>(DesfireError::FileNotFound));
		default:
		    return R::Err(Error<PcscError>(DesfireError::Generic).detail("Unknown status word: ").meta("sw", sw));
	}
}
